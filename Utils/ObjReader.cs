    ...
    internal class ObjReader : ModelReader
    {
            ...
            while (!materialReader.EndOfStream)
            {
                ...
                switch (keyword.ToLower())
                {
                    case "newmtl":
                        if (value != null)
                        {
                            ...
                            else
                            {
                                ...
                                currentMaterial.s3_bucket_url = s3_bucket_url;
                                currentMaterial.model = model;
                    ...
                    case "map_ka":
                        ...
                        {
                            SingleModel.Texture textureFound = model.textures.Find(texture => texture.file.metadata.file_name.Equals(value));
        ...
        public class MaterialDefinition
        {
            ...
            private Material CreateMaterial(string texturePath)
            {
                ...
                {
                    mg.Children.Add(new DiffuseMaterial(this.CreateTextureBrush(this.DiffuseMap, this.model.updatedAt)));
                }
                ...
                {
                    mg.Children.Add(new EmissiveMaterial(this.CreateTextureBrush(this.AmbientMap, this.model.updatedAt)));
                }
            ...
            private ImageBrush CreateTextureBrush(string fileName, string updatedAt)
            {
                SingleModel.Texture _texture = model.textures.Find(texture => texture.file.metadata.file_name.Equals(fileName));
                BitmapImage img = StreamLoader.LoadTexture($@"{model.id}\texture\{_texture.file.metadata.file_name}", $@"{s3_bucket_url}/{_texture.file.path}", updatedAt);
                ImageBrush textureBrush = new ImageBrush(img) { Opacity = Dissolved, ViewportUnits = BrushMappingMode.Absolute, TileMode = TileMode.Tile };
                return textureBrush;
            }
        }
